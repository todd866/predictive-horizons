...
\begin{abstract}
The OpenWorm project reconstructed the complete \textit{C.~elegans} connectome---classically 302 neurons and ${\sim}7{,}000$ synapses, expanded to 448 neurons in the full hermaphrodite dataset \cite{varshney2011}---and simulated it in silico, yet the resulting model cannot produce the coordinated locomotion that the living worm achieves trivially. We argue that this failure is not computational but ontological: the connectome is a low-dimensional measurement of a high-dimensional biological system, not the system itself. Every neuron in the worm sits in a continuous bioelectric field---ephaptic coupling, neuropeptide broadcast, proprioceptive feedback---that the wiring diagram does not capture. We develop a synchronization-based framework for biological prediction organized around three timescales ($\tsync < \tmeas < \tenv$) and test it on the real \textit{C.~elegans} connectome. In fixed-coupling simulations, mutual information with the environment is similar across models (${\sim}0.47$ nats), but field coupling doubles the order parameter and produces reliable motor coordination (VA-VB $= 0.85 \pm 0.08$ with ephaptic field versus $0.47 \pm 0.29$ without). The connectome carries information; the field coordinates the body. We then construct a Landau-Ginzburg coherence field model in which the coupling itself is a dynamical variable $\varphi(x,t)$ on the worm's body axis, evolving through a double-well potential with surface tension at coherence boundaries. The coherence field produces an emergent anterior-posterior gradient (sensory neurons pump coherence at the head), local effective dimensionality that anti-correlates with coherence (coherent regions have compressed, structured dynamics), and motor coordination that arises from the $\varphi$-synchrony feedback loop rather than from imposed coupling constants. The coherence domain is low-dimensional at macroscale (its gross shape requires few parameters) but high-dimensional at microscale (rich phase relationships among hundreds of coupled oscillators within). Low-dimensional defence mechanisms (the potential landscape, homeostatic attractors) maintain the high-dimensional interior against noise. This replaces the Markov blanket framework with something physically concrete: the boundary is a graded coherence gradient with measurable energetic cost, not a statistical partition over discrete variables. We show that low-dimensional observers systematically underestimate the information high-dimensional systems contain, and that discrete coding destroys ${\sim}85\%$ of predictive content. Intelligence and prediction live upstream of code, in the coherence dynamics, not in the coded outputs.
\end{abstract}

\noindent\textbf{Keywords:} coherence field, connectome, Landau-Ginzburg, motor coordination, dimensionality, prediction, synchronization, \textit{C.~elegans}, phase transition
...
\textit{Simulation models.} Two simulation approaches are presented. (1)~\textit{Fixed-coupling models} (Section~\ref{sec:three_models}): 448 coupled phase oscillators on the real connectome \cite{varshney2011}, with neuron positions from WormAtlas \cite{altun2024}. Three models (connectome only, +ephaptic, +full field) at three coupling regimes with five seeds each. $dt = 0.001$\,s, $T = 50$\,s, MI estimated via Gaussian covariance. (2)~\textit{Coherence field model} (Section~\ref{sec:coherence_field}): a Landau-Ginzburg order parameter $\varphi(x,t)$ on the body axis (200 grid points, $dt = 0.0002$\,s, $T = 200$\,s), bidirectionally coupled to the same 448-neuron connectome. The coherence field evolves by Eq.~\eqref{eq:coherence} with potential $V(\varphi) = -(a/2)\varphi^2 + (b/4)\varphi^4$ ($a = 0.5$, $b = 2.0$), gradient smoothing ($\kappa = 0.02$), and neural synchrony source term. The dynamics are graded: coherence varies continuously with the local balance between pump and noise. The oscillator field simulation (Figures~\ref{fig:alignment}, \ref{fig:gap}, \ref{fig:code_collapse}) uses a $25 \times 25$ lattice with nearest-neighbor coupling ($K_{\mathrm{nn}} = 2$) driven by a spatiotemporal OU environment ($\tenv = 4$\,s, spatial correlation $L = 4$). Code is available at \url{https://github.com/todd866/predictive-horizons}.
...
\subsection{Thermodynamics of coherence: substrates, metabolism, and aging}
\label{sec:substrates}

The coherence field model makes the thermodynamics of intelligence concrete. Maintaining a coherence bubble costs energy: the neural synchrony that pumps $\varphi$ requires metabolic investment, and the surface tension at coherence boundaries represents a continuous thermodynamic cost proportional to $\kappa |\nabla\varphi|^2$. A coherent system occupies a lower free energy state than a decoherent one (the potential well at $\varphi^*$ is below $\varphi = 0$), but reaching and maintaining that state requires work against noise. Intelligence, in this framework, is the maintenance of coherence against thermal degradation---a thermodynamic achievement, not a computational one.

This has direct implications for aging. Cohen et al.\ \cite{cohen2022} documented progressive loss of physiological complexity with age: the degradation of high-dimensional oscillatory structure in cardiac, neural, and hormonal dynamics. In the coherence field framework, aging is the contraction of coherence domains. As metabolic capacity declines, the organism can no longer sustain the neural synchrony that pumps $\varphi$. The pump weakens, noise dominates over a larger fraction of the body axis, and the coherent domain shrinks toward the most densely innervated regions. The organism's $\Deff$ decreases not because the system simplifies but because coherence can no longer be maintained at the same spatial extent. Prediction degrades because the coherence domain no longer spans enough of the nervous system to coordinate adaptive behavior.

The framework also implies that biological intelligence requires a fundamentally different substrate than discrete computation. An oscillatory substrate maintains coherence natively---it re-entrains coupled elements every cycle, sustaining the synchronized manifold without explicit computation. A discrete substrate must \textit{simulate} this coupling through sequential updates, paying an additional thermodynamic cost for each discrete approximation to continuous dynamics.
...
The connectome is a remarkable scientific achievement---a complete wiring diagram of a nervous system. But a wiring diagram is a measurement, not a system. The \textit{C.~elegans} result demonstrates this concretely: the connectome carries environmental information but cannot coordinate the body. The coherence field model shows what the connectome misses: a continuous, dynamic order parameter whose spatiotemporal structure \textit{is} the organism's coordination capacity. The field is not an add-on to the connectome; it is the primary dynamical variable, with the connectome providing the anatomical skeleton around which coherence organizes.

The coherence domain---a spatially graded region of high $\varphi$, low-dimensional at macroscale, high-dimensional at microscale, maintained against noise by the neural synchrony pump---is a more physically precise object than the Markov blanket. Its gradients have measurable energetic cost ($\kappa |\nabla\varphi|^2$). Its interior has quantifiable information geometry. Its persistence requires metabolic investment. And its contraction with metabolic decline provides a concrete, falsifiable account of why aging degrades cognitive and motor function.

Prediction is the most useful thing biology does with high-dimensional dynamics. It arises not from computation over internal models but from the maintenance of coherence---the thermodynamic work of keeping a high-dimensional system on a structured, low-dimensional manifold against the constant pressure of thermal noise. Intelligence lives upstream of code, in the coherence dynamics that generate predictions through synchronization. The field is right there. Evolution tunes it. Our simulations should model it.
