...
\end{equation}
where $V(\varphi) = -\frac{a}{2}\varphi^2 + \frac{b}{4}\varphi^4$ is a double-well potential with stable minima at $\varphi = 0$ (decoherent) and $\varphi^* = \sqrt{a/b}$ (coherent), $\kappa \nabla^2 \varphi$ is a diffusion term whose coefficient $\kappa$ sets the surface tension at coherence boundaries, $S(x,t)$ is a source term driven by local neural synchrony, and $\eta$ is spatiotemporal noise.
...
\paragraph{Results.} At parameters tuned to the critical regime ($a = 0.5$, $b = 2.0$, $\kappa = 0.02$, $\sigma_\varphi = 0.4$), the coherence field produces spatially structured dynamics (Figure~\ref{fig:coherence_field}).
...
\item \textbf{Surface tension at coherence boundaries.} The gradient energy $|\nabla\varphi|^2$, which quantifies the thermodynamic cost of maintaining coherence boundaries, peaks in the mid-body region where the coherent anterior meets the less-coherent posterior. This is a real phase boundary with measurable surface tension---the energetic cost of maintaining the coherence gradient.
...
\paragraph{Interpretation: the coherence bubble.} The coherence field provides a thermodynamic framework for the coordination result. Inside a region of high $\varphi$ (a ``coherence bubble''), the dynamics are structured, low-dimensional at macroscale, and coordinated. Outside, they are noisy, high-dimensional, and uncoordinated. The boundary between these regions has surface tension $\sigma \propto \kappa |\nabla\varphi|^2$, and the bubble persists only when the bulk free energy gain from coherence (the depth of the potential well) exceeds the surface tension cost of maintaining the boundary. This defines a \textit{critical radius}: the minimum spatial extent of coherence that can self-sustain through the synchrony-coupling feedback loop. Below this radius, noise collapses the bubble; above it, the bubble recruits adjacent regions and coordination propagates.

The double-well potential $V(\varphi)$ plays the role of a low-dimensional defence mechanism: it stabilizes the coherent state once established, requiring a finite perturbation to push the system back to decoherence. But the dynamics \textit{within} the coherent bubble are high-dimensional---hundreds of coupled oscillators with rich phase relationships, multi-scale texture, and information content that far exceeds the few parameters needed to describe the bubble's macroscopic shape. The bubble is low-dimensional at macroscale and high-dimensional at microscale: a high-dimensional interior protected by a low-dimensional boundary.
...
\textit{Simulation models.} Two simulation approaches are presented. (1)~\textit{Fixed-coupling models} (Section~\ref{sec:three_models}): 448 coupled phase oscillators on the real connectome \cite{varshney2011}, with neuron positions from WormAtlas \cite{altun2024}. Three models (connectome only, +ephaptic, +full field) at three coupling regimes with five seeds each. $dt = 0.001$\,s, $T = 50$\,s, MI estimated via Gaussian covariance. (2)~\textit{Coherence field model} (Section~\ref{sec:coherence_field}): a Landau-Ginzburg order parameter $\varphi(x,t)$ on the body axis (200 grid points, $dt = 0.0002$\,s, $T = 200$\,s), bidirectionally coupled to the same 448-neuron connectome. The coherence field evolves by Eq.~\eqref{eq:coherence} with double-well potential ($a = 0.5$, $b = 2.0$), surface tension ($\kappa = 0.02$), and neural synchrony source term. The oscillator field simulation (Figures~\ref{fig:alignment}, \ref{fig:gap}, \ref{fig:code_collapse}) uses a $25 \times 25$ lattice with nearest-neighbor coupling ($K_{\mathrm{nn}} = 2$) driven by a spatiotemporal OU environment ($\tenv = 4$\,s, spatial correlation $L = 4$). Code is available at \url{https://github.com/todd866/predictive-horizons}.
...
This has direct implications for aging. Cohen et al.\ \cite{cohen2022} documented progressive loss of physiological complexity with age: the degradation of high-dimensional oscillatory structure in cardiac, neural, and hormonal dynamics. In the coherence field framework, aging is the shrinkage of coherence bubbles. As metabolic capacity declines, the organism can no longer sustain the neural synchrony that pumps $\varphi$, the coherent wells become shallower relative to noise, and the critical radius for self-sustaining coherence grows. Bubbles that were previously stable collapse; the organism's $\Deff$ decreases not because the system simplifies but because coherence can no longer be maintained at the same spatial extent. Prediction degrades because the coherence bubble no longer spans enough of the nervous system to coordinate adaptive behavior.
...
The coherence bubble---a spatially localized region of high $\varphi$, low-dimensional at macroscale, high-dimensional at microscale, defended by the double-well potential and maintained against noise by metabolic expenditure---is a more physically precise object than the Markov blanket. Its boundary has measurable surface tension. Its interior has quantifiable information geometry. Its persistence has a thermodynamic cost. And its collapse with metabolic decline provides a concrete, falsifiable account of why aging degrades cognitive and motor function.
