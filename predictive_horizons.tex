...
Moreover, just because the principal axes of neural dynamics span a 10-dimensional space does not mean that space is homogeneous. Within the low-$\Deff$ manifold there is microstructure: sub-partitions, basins of attraction, separatrices between metastable states, and transient trajectories connecting them. The sequential behavioral states that Kato et al.\ \cite{kato2015} observed---forward locomotion, reversal, turning, dwelling---are exactly such sub-partitions within the low-dimensional manifold. PCA captures the \textit{envelope} of the dynamics (the overall volume the system visits); it does not capture the internal organization of that volume. The field coupling is what structures the interior: without it, the $\Deff \approx 24$ system fills the accessible volume diffusely, visiting motor states without clear transitions. With it, the $\Deff \approx 10$ system is partitioned into functional regions with stereotyped transitions between them---the spatially graded ephaptic coupling creates the travelling-wave coordination that distinguishes forward locomotion from reversal. The field does not shrink the state space; it partitions it into a motor program.
